...
input_file_name = './scripts/CaseFolding.txt'
txt = open(input_file_name)
...
try:
    ...
    second_line_match = re.search(r"^# Date: (\d{4}-\d{2}-\d{2})", lines[1])
    unicode_version_date = second_line_match[1]
except TypeError:
    print('Did not find Unicode version number on first line of %s or date of release on second line' % input_file_name, file = stderr)
    exit(code = 1)
...
rs.write('// Case-folding function for Unicode %s (released on %s)\n' % (unicode_version, unicode_version_date))
rs.write('// Generated by scripts/mapgen.py on %s\n' % datetime.date.today())
rs.write('\n')
